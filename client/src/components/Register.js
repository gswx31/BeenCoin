...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success(referralCode ? '🎁 50,000 보너스 받았어요! 로그인해주세요' : '회원가입 완료! 로그인해주세요 🎊');
      navigate('/login');
    } catch (error) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-dark-900 px-4">
      <div className="w-full max-w-sm fade-in">
        <div className="text-center mb-8">
          ...
          <p className="text-muted text-sm">$1,000,000 가상 자금으로 시작!</p>
        </div>
        <div className="bg-dark-800 rounded-3xl p-8 border border-dark-600">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              <label className="block text-muted text-xs font-medium mb-2">
                추천 코드 <span className="text-mint">(선택 · +$50,000 보너스)</span>
              </label>
              <input type="text" value={referralCode}
                onChange={(e) => setReferralCode(e.target.value.toUpperCase())}
                maxLength={8}
                className="w-full px-4 py-3 bg-dark-700 border border-dark-600 rounded-2xl text-white font-mono uppercase tracking-wider placeholder-dark-500 focus:outline-none focus:border-accent transition-all"

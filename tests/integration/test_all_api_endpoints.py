...
class TestAuthAPI:
    ...
    def test_register_invalid_username_special_char(self, client: TestClient):
        ...
        response = client.post(
            "/api/v1/auth/register", json={"username": "user_name", "password": "testpass123"}
        )
    ...
    def test_register_invalid_username_too_short(self, client: TestClient):
        ...
        response = client.post(
            "/api/v1/auth/register", json={"username": "ab", "password": "testpass123"}
        )
        assert response.status_code == 422
    ...
    def test_register_invalid_password_too_short(self, client: TestClient):
        """너무 짧은 비밀번호로 회원가입 실패"""
        response = client.post(
            "/api/v1/auth/register", json={"username": "validuser", "password": "short"}
        )
        assert response.status_code == 422

    def test_register_invalid_data(self, client: TestClient):
        """잘못된 데이터로 회원가입 실패"""
        response = client.post("/api/v1/auth/register", json={"username": ""})
        assert response.status_code == 422
    ...
    def test_login_wrong_password(self, client: TestClient, test_user):
        """잘못된 비밀번호로 로그인 실패"""
        response = client.post(
            "/api/v1/auth/login", data={"username": test_user.username, "password": "wrongpassword"}
        )
    ...
    def test_login_nonexistent_user(self, client: TestClient):
        """존재하지 않는 사용자로 로그인 실패"""
        response = client.post(
            "/api/v1/auth/login", data={"username": "nonexistentuser", "password": "testpass123"}
        )
        assert response.status_code == 401
    ...
    def test_protected_endpoint_with_expired_token(self, client: TestClient, expired_headers):
...
class TestMarketAPI:
    ...
    def test_get_historical_data(self, client: TestClient):
        ...
        wait_for_api()
        response = client.get(
            "/api/v1/market/historical/BTCUSDT", params={"interval": "1h", "limit": 24}
        )
        assert response.status_code == 200
        data = response.json()
        assert isinstance(data, list)
    ...
    def test_get_recent_trades(self, client: TestClient):
        """최근 거래 내역 조회"""
        wait_for_api()
        response = client.get("/api/v1/market/trades/BTCUSDT", params={"limit": 20})
        assert response.status_code == 200
        data = response.json()
        assert isinstance(data, list)
...
class TestFuturesAPI:
    ...
    def test_open_long_position(self, client: TestClient, auth_headers):
        ...
        if response.status_code == 200:
            ...
            assert any(
                keyword in error_detail.lower() for keyword in ["잔고", "증거금", "부족", "balance"]
            )
        else:
            # 500 에러 등 예상치 못한 상황
    ...
    def test_get_open_positions(self, client: TestClient, auth_headers):
        ...
        wait_for_api()
        response = client.get(
            "/api/v1/futures/positions", params={"status": "OPEN"}, headers=auth_headers
        )
        assert response.status_code == 200
    ...
    def test_get_closed_positions(self, client: TestClient, auth_headers):
        ...
        wait_for_api()
        response = client.get(
            "/api/v1/futures/positions", params={"status": "CLOSED"}, headers=auth_headers
        )
        assert response.status_code == 200
        assert isinstance(response.json(), list)
    ...
    def test_get_futures_transactions(self, client: TestClient, auth_headers):
        ...
        wait_for_api()
        response = client.get(
            "/api/v1/futures/transactions", params={"limit": 10}, headers=auth_headers
        )
        assert response.status_code == 200
        assert isinstance(response.json(), list)
    ...
    def test_close_position_flow(self, client: TestClient, auth_headers):
        ...
        if open_response.status_code == 200:
            ...
            close_response = client.post(
                f"/api/v1/futures/positions/{position_id}/close", headers=auth_headers
            )
    ...
    def test_close_nonexistent_position(self, client: TestClient, auth_headers):
        ...
        response = client.post(f"/api/v1/futures/positions/{fake_id}/close", headers=auth_headers)
...
class TestPortfolioAPI:
    ...
    def test_get_portfolio_transactions(self, client: TestClient, auth_headers):
        ...
        wait_for_api()
        response = client.get(
            "/api/v1/futures/portfolio/transactions", params={"limit": 20}, headers=auth_headers
        )
        assert response.status_code == 200
        assert isinstance(response.json(), list)
    ...
    def test_get_portfolio_stats(self, client: TestClient, auth_headers):
        """거래 통계 조회"""
        wait_for_api()
        response = client.get("/api/v1/futures/portfolio/stats", headers=auth_headers)
        assert response.status_code == 200
        data = response.json()
...
class TestErrorCases:
    ...
    def test_missing_required_field(self, client: TestClient):
        """필수 필드 누락"""
        response = client.post("/api/v1/auth/register", json={"username": "validuser123"})
        assert response.status_code == 422
...
class TestE2EScenarios:
    ...
    def test_complete_user_registration_and_login(self, client: TestClient):
        ...
        login_response = client.post(
            "/api/v1/auth/login", data={"username": username, "password": "testpass123"}
        )
    ...
    def test_complete_trading_flow(self, client: TestClient):
        ...
        if position_response.status_code == 200:
            ...
            positions_response = client.get(
                "/api/v1/futures/positions", params={"status": "OPEN"}, headers=headers
            )
            ...
            wait_for_api()
            close_response = client.post(
                f"/api/v1/futures/positions/{position_id}/close", headers=headers
            )
            assert close_response.status_code in [200, 400]

        # 7. 거래 내역 확인 (항상 가능)
        wait_for_api()
        transactions_response = client.get("/api/v1/futures/transactions", headers=headers)
    ...
    def test_user_isolation(self, client: TestClient, user_factory):
        ...
        """
        # 사용자 1 생성 및 로그인
        user1 = user_factory(username=generate_valid_username("user1"), password="password123")
        login1 = client.post(
            "/api/v1/auth/login", data={"username": user1.username, "password": "password123"}
        )
        headers1 = {"Authorization": f"Bearer {login1.json()['access_token']}"}

        # 사용자 2 생성 및 로그인
        user2 = user_factory(username=generate_valid_username("user2"), password="password456")
        login2 = client.post(
            "/api/v1/auth/login", data={"username": user2.username, "password": "password456"}
        )

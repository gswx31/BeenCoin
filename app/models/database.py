...
class TradingAccount(SQLModel, table=True):
    ...
    balance: Decimal = Field(
        default=Decimal("100000"), max_digits=20, decimal_places=8, description="사용 가능한 잔액"
    )
    ...
    # 수익 정보
    total_profit: Decimal = Field(default=Decimal("0"), max_digits=20, decimal_places=8)

    created_at: datetime = Field(default_factory=datetime.utcnow)
    updated_at: datetime = Field(default_factory=datetime.utcnow)

    # Relationships
    user: User | None = Relationship(back_populates="trading_account")
...
class Order(SQLModel, table=True):
    ...
    price: Decimal | None = Field(
        default=None, max_digits=20, decimal_places=8, description="지정가 (시장가는 None)"
    )
    ...
    average_price: Decimal | None = Field(default=None, max_digits=20, decimal_places=8)

    # 수수료
    fee: Decimal | None = Field(default=None, max_digits=20, decimal_places=8)
...
class Position(SQLModel, table=True):
    ...
    # Relationships
    account: TradingAccount | None = Relationship(back_populates="positions")
...
class Transaction(SQLModel, table=True):
    ...
    # 실현 손익 (매도 시에만)
    realized_profit: Decimal | None = Field(default=None, max_digits=20, decimal_places=8)
...
class PriceAlert(SQLModel, table=True):
    ...
    condition: str = Field(max_length=10, description="ABOVE(이상) 또는 BELOW(이하)")

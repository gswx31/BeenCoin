...
async def execute_market_order_with_real_trades(
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail="시장가 주문 실행 실패")
    ...
    try:
        async with httpx.AsyncClient(timeout=TIMEOUT) as client:
            ...
            if response.status_code == 200:
                ...
                logger.warning(
                    f"⚠️ 24h 티커 조회 실패: {symbol} - " f"Status {response.status_code}"
                )
                return None
    ...
    try:
        async with httpx.AsyncClient(timeout=TIMEOUT) as client:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"✅ 과거 데이터 조회 성공: {symbol} - " f"{len(formatted_data)}개")
                return formatted_data
            elif response.status_code == 451:
                logger.error("❌ 지역 제한: Binance API 접근 불가 (451)")
                return []
            else:
                logger.error(f"❌ 과거 데이터 조회 실패: " f"Status {response.status_code}")
                return []
    ...
    try:
        async with httpx.AsyncClient(timeout=TIMEOUT) as client:
            ...
            if response.status_code == 200:
                ...
                logger.error(f"❌ Binance API 연결 실패: " f"Status {response.status_code}")
                return False

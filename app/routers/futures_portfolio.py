...
class TransactionDetail(BaseModel):
    ...
    fills: Optional[List[FillDetail]] = None
...
async def get_position_fills(
    ...
    try:
        ...
        transactions = session.exec(
            select(FuturesTransaction).where(
                FuturesTransaction.position_id == position_id
...
async def get_transactions(
    ...
    action: Optional[str] = Query(
        None,
        description="액션 필터 (OPEN, CLOSE, LIQUIDATION)"
    ),
    ...
    try:
        # 거래 내역 조회
        query = select(FuturesTransaction).where(
            FuturesTransaction.user_id == current_user.id
        )
